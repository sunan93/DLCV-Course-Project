...
def get_CP_loss(net_list, targets_feature_list, poison_batch, s_coeff_list, coeffs_fixed, net_repeat, multi_flag=False, tol=1e-6):
    """
    ...
    for net in net_list:
        if net_repeat > 1:
            poisons = [net(x=poison_batch(), penu=True, multi=multi_flag) for _ in range(net_repeat)]
            poisons = sum(poisons) / len(poisons)
        elif net_repeat == 1:
            poisons = net(x=poison_batch(), penu=True, multi=multi_flag)
        else:
            assert False
...
def get_CP_loss_end2end(net_list, targets_feature_list, poison_batch, s_coeff_list, coeffs_fixed, net_repeat, multi_flag=False, tol=1e-6):
    """
    Corresponding to one step of the outer loop (except for updating and clipping) of Algorithm 1
    """
    #print(multi_flag)
    poison_feat_mat_list = [net(x=poison_batch(), block=True, multi=multi_flag) for net in net_list]
  
    #print(len(poison_feat_mat_list), len(poison_feat_mat_list[0]), poison_feat_mat_list[0][0].shape)
    total_loss = 0
    coeffs_time = 0
...
def loss_from_center(subs_net_list, target_feat_list, poison_batch, antidote_batch, net_repeat, end2end, max_loss_flag=False, feat_reg_flag=False, ce_loss_flag=False, multi_flag=False, antidote_flag=False, device='cuda'):
    if end2end:
        loss = 0
        loss_arr = []
        loss_coef = []
        for i in range(len(subs_net_list)):
            loss_coef.append(float(i)/len(subs_net_list))
        for net, center_feats in zip(subs_net_list, target_feat_list):
            if net_repeat > 1:
                ...
                poisons_feats = net(x=poison_batch(), block=True, multi=multi_flag)
                antidote_feats = net(x=antidote_batch(), block=True, multi=multi_flag)
                poisons_outs = net(x=poison_batch(), block=False)
                desired_outs = torch.zeros([len(poison_batch())]).to(device).long()
                desired_outs[:]=1
            else:
                assert False, "net_repeat set to {}".format(net_repeat)
            
            net_loss = 0
            for Afeat, pfeat, cfeat in zip(antidote_feats, poisons_feats, center_feats):
                if feat_reg_flag==True:
                    cfeat = cfeat.to(device)
                    diff = torch.mean(pfeat - cfeat, dim=0)
                    div_feat = torch.mean(cfeat, dim=0)
                    diff_norm = torch.norm(diff, dim=0) / torch.norm(div_feat, dim=0)
                else:
                    #print(pfeat.shape, cfeat.shape)
                    diff = torch.mean(pfeat, dim=0) - cfeat
                    diff_norm = torch.norm(diff, dim=1) / torch.norm(cfeat, dim=1)
                    if antidote_flag:
                        #diff_2 = torch.mean(Afeat, dim=0) - cfeat
                        #diff2_norm = -0.1 * torch.norm(diff_2, dim=1) / torch.norm(cfeat, dim=1)
                        antidote_outs = net(x=antidote_batch(), block=False)
                        cross_entropy_criterion = nn.CrossEntropyLoss()
                        ant_desired_outs = torch.zeros([len(antidote_batch())]).to(device).long()
                        ant_desired_outs[:]=1
                        ce_loss_antidote = 0.1*cross_entropy_criterion(antidote_outs, ant_desired_outs)

                net_loss += torch.mean(diff_norm)
                if antidote_flag:
                    #net_loss += torch.mean(diff2_norm)
                    net_loss += ce_loss_antidote

            cross_entropy_loss = nn.CrossEntropyLoss()
            ce_loss = cross_entropy_loss(poisons_outs, desired_outs)
            loss += net_loss / len(center_feats)
            if ce_loss_flag==True:
                loss += 0.1*ce_loss
            if max_loss_flag==True:
                loss_arr.append(net_loss/len(center_feats))
        if max_loss_flag==True:
            loss_arr.sort(reverse=True)
            loss_coef.sort(reverse=True)
            #print(loss_arr, loss_coef)
            for j in range(len(loss_arr)):
                loss_arr[j] = loss_arr[j]*loss_coef[j]
            #loss = sum(loss_arr[0:int(len(loss_arr)/2)]) / (int(len(loss_arr)/2))
            loss = sum(loss_arr)
        else:
        ...
        for net, center in zip(subs_net_list, target_feat_list):
            ...
            antidote_feats = [net(x=antidote_batch(), penu=True) for _ in range(net_repeat)] 
            poisons_outs = net(x=poison_batch(), block=False)
            desired_outs = torch.zeros([len(poison_batch())]).to(device).long()
            desired_outs[:]=1
            ...
            if feat_reg_flag==True:
                print(poisons.shape, center.shape)
                diff = torch.mean(poisons - center, dim=0)
                diff_norm = torch.norm(diff, dim=0) / torch.norm(center, dim=0)
            else:
                diff = torch.mean(poisons, dim=0) - center
                diff_norm = torch.norm(diff, dim=1) / torch.norm(center, dim=1)
                if antidote_flag:
                    diff_2 = torch.mean(antidote_feats, dim=0) - center
                    diff2_norm = (-1) * torch.norm(diff_2, dim=1) / torch.norm(center, dim=1)
...
def make_convex_polytope_poisons(subs_net_list, victim_net, base_tensor_list, base_antidote_tensor_list, targets, targets_indices, device,
                                 opt_method='adam',
                                 ...
                                 chk_path='', poison_idxes=[], antidote_idxs=[], poison_label=-1,target_label=-1,
                                 tol=1e-6, start_ite=0, poison_init=None, antidote_init=None,end2end=None, mode=None,
                                 net_repeat=1, max_loss_flag=False, feat_reg_flag=False, ce_loss_flag=False, multi_flag=False, antidote_flag=False):
    victim_net.eval()

    poison_batch = PoisonBatch(poison_init).to(device)
    #if antidote_flag:
    antidote_batch = PoisonBatch(antidote_init).to(device)

    learnable_params = list(poison_batch.parameters())
    if antidote_flag:
        learnable_params += list(antidote_batch.parameters())

    opt_method = opt_method.lower()
    if opt_method == 'sgd':
        optimizer = torch.optim.SGD(learnable_params, lr=lr, momentum=momentum)
    elif opt_method == 'signedadam':
        optimizer = SignedAdam.SignedAdam(learnable_params, lr=lr, betas=(momentum, 0.999))
        print("Using Signed Adam")
    elif opt_method == 'adam':
        optimizer = torch.optim.Adam(learnable_params, lr=lr, betas=(momentum, 0.999))
    ...
    if antidote_flag:
        base_antidote_tensor_batch = torch.stack(base_antidote_tensor_list, 0)
        base_antidote_range01_batch = base_antidote_tensor_batch * std + mean

    targets_feat_list = get_targets_feat_list(subs_net_list, targets, device, multi_flag, end2end)
    targets_feat_in_victim = get_targets_feat_list([victim_net], targets, device, multi_flag, end2end)[0]

    #print(len(targets_feat_list), len(targets_feat_list[0]), len(targets_feat_list[0][0]), targets_feat_list[0][0].shape)
    ...
    for n, net in enumerate(subs_net_list):
        net.eval()
        if end2end:
            block_feats = [feat.detach() for feat in net(x=targets[0], block=True, multi=multi_flag)]
    ...
    if end2end:
        block_feats = [feat.detach() for feat in victim_net(x=targets[0], block=True, multi=multi_flag)]
    ...
    if mode in ['mean', 'convex-mean']:
        if end2end:
            ...
            if feat_reg_flag==False:
                ...
                # 12 x T x 2 x [feat_dim] --> 12 x 2 x P x [feat_dim]
                for i, net in enumerate(subs_net_list):
                    center_list = []
                    for k in range(len(targets_feat_list[i][0])):
                        #print(i, k)
                        #print( targets_feat_list[i][0][k].shape)
                        if len(targets_feat_list[i][0][k].shape)==4:
                            x0, x1,x2,x3 = targets_feat_list[i][0][k].shape
                            T = torch.zeros(n_targets, x1*x2*x3)
                        else:
                            x0, x1 = targets_feat_list[i][0][k].shape
                            T = torch.zeros(n_targets, x1)
                        #print("T.shape ", T.shape)
                        for j in range(len(targets_feat_list[0])):
                            x = torch.flatten(targets_feat_list[i][j][k])    
                            #print("x.shape ", x.shape)
                            T[j] = x
                        #print("kmeans : ", T.shape)
                        target_ids, centers = kmeans(
                        X=T, num_clusters=n_poisons, distance='euclidean', device=torch.device('cuda'))
                        #print("kmeans:", centers.shape)
                        if len(targets_feat_list[i][0][k].shape)==4:
                            centers = torch.reshape(centers, (n_poisons,x1,x2,x3))
                        else:
                            centers = torch.reshape(centers, (n_poisons,x1))

                        center_list.append(centers)
                    targets_center_list.append(center_list)
    ...
    for ite in range(start_ite, iterations):
        ...
        if antidote_flag==True:
            antidote_batch.zero_grad()
        t = time.time()
        if mode == 'convex':
            ...
            if feat_reg_flag==True:
                total_loss = loss_from_center(subs_net_list, targets_center_list, poison_batch, net_repeat, end2end, False, feat_reg_flag, False, multi_flag)
            elif ce_loss_flag==True:
                total_loss = loss_from_center(subs_net_list, targets_center_list, poison_batch, net_repeat, end2end, False, False, True, multi_flag)
            elif max_loss_flag==True:
                total_loss = loss_from_center(subs_net_list, targets_center_list, poison_batch, net_repeat, end2end, True, False, False, multi_flag)
            else:
                total_loss = loss_from_center(subs_net_list, targets_center_list, poison_batch, antidote_batch, net_repeat, end2end, False, False, False, multi_flag, antidote_flag)
            coeffs_time_tmp = 0
        ...
        if antidote_flag:
            perturb_antidote_range01 = torch.clamp((antidote_batch.poison.data - base_antidote_tensor_batch) * std, -epsilon, epsilon)
            perturbed_antidote_range01 = torch.clamp(base_antidote_range01_batch.data + perturb_antidote_range01.data, 0, 1)
            antidote_batch.poison.data = (perturbed_antidote_range01 - mean) / std
            #print(torch.sum(perturb_antidote_range01), torch.sum(perturb_range01))
    ...
    return get_poison_tuples(poison_batch, poison_label), get_poison_tuples(antidote_batch, target_label)
...
def train_network_with_poison(net, target_imgs, targets_indices, poison_tuple_list, antidote_tuple_list, base_idx_list, base_antidotes_idx_list, chk_path, args,
                              save_state=False, eval_targets=None, device='cuda', load_poisons=1):
    ...
    print(base_idx_list, base_antidotes_idx_list)
    poisoned_dset = PoisonedDataset(args.train_data_path, subset='others', transform=transform_train,
                                    ...
                                    poison_indices=base_idx_list, antidote_tuple_list=antidote_tuple_list,
                                    antidote_indices=base_antidotes_idx_list,subset_group=args.subset_group, load_poisons=load_poisons)
    ...
    # Evaluate test accuracy of pretrained model on test data
    print("-----------------------------------------------")
    print("Now evaluating pretrained network on the clean test set")
    Test_acc_meter = AverageMeter()
    per_class_acc_meter = np.zeros(10)
    iters = 0
    with torch.no_grad():
        for ite, (input, target) in enumerate(test_loader):
            ...
            per_class_acc_meter += np.array(per_class_acc)
            Test_acc_meter.update(prec1.item(), input.size(0))

            if ite % 100 == 0 or ite == len(test_loader) - 1:
                print("{1}, Val iteration {0}, "
                      "acc {acc.val:.3f} ({acc.avg:.3f})".
                      format(ite, time.strftime("%Y-%m-%d %H:%M:%S"), acc=Test_acc_meter))
            iters+=1
    per_class_acc_meter /= iters

    print("* Prec: {}".format(Test_acc_meter.avg))
    print("Per class accuracy = ", per_class_acc_meter)
    print("-----------------------------")


    print("------------------------------------------------")
    print("--- Antidote and poison preditions on pretrained model ---")
 
    antidote_pred_list = []
    for antidote_img, _ in antidote_tuple_list:
        antidote_scores = net(antidote_img[None, :, :, :].to(device))
        antidote_score, antidote_pred = antidote_scores.topk(1, 1, True, True)
        antidote_pred_list.append(antidote_pred.item())
    print("Target label: {}, Poison label: {}, Antidotes' Predictions:{}"
              .format(args.target_label, args.poison_label, antidote_pred_list))
    ...
    for idx, target_img in zip(targets_indices, target_imgs):
        ...
        target_pred_list.append(pred.item())
    print("Target's Score:{}".format(target_pred_list))
    ...
    return targets_acc_meter.avg, eval_targets_acc_meter.avg, Test_acc_meter.avg, val_acc_meter.avg
