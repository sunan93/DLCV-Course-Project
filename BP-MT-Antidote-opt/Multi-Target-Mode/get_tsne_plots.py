...
if __name__ == '__main__':
    ...
    print(targets.shape, car_imgs.shape, frog_imgs.shape)
    poisons_list = torch.load("poison_list.pt")
    antidotes_list = torch.load("antidote_list.pt")
    poisons_init = torch.load("poison_init.pt")
    antidotes_init = torch.load("antidote_init.pt")

    poison_imgs = [poisons_list[i][0] for i in range(len(poisons_list))]
    poison_imgs = torch.stack(poison_imgs).to(args.device)
    antidote_imgs = [antidotes_list[i][0] for i in range(len(antidotes_list))]
    antidote_imgs = torch.stack(antidote_imgs).to(args.device)
 
    print("poison init = ", poisons_init[0].shape) 
    poison_init = [poisons_init[i] for i in range(len(poisons_init))]
    poison_init = torch.stack(poison_init).to(args.device)
    antidote_init = [antidotes_init[i] for i in range(len(antidotes_init))]
    antidote_init = torch.stack(antidote_init).to(args.device)
 
    print(poison_imgs.shape, antidote_imgs.shape, poison_init.shape, antidote_init.shape)
    with torch.no_grad():
        for n_net, net in enumerate(sub_net_list):
            ...
            poison_imgs_feat = net.module.penultimate(poison_imgs)
            poison_init_feat = net.module.penultimate(poison_init)
            antidote_imgs_feat = net.module.penultimate(antidote_imgs)
            antidote_init_feat = net.module.penultimate(antidote_init)
            break
    print(target_img_feat.shape, car_imgs_feat.shape, frog_imgs_feat.shape, poison_imgs_feat.shape, poison_init_feat.shape, antidote_imgs_feat.shape, antidote_init_feat.shape)

    px = torch.cat((frog_imgs_feat, car_imgs_feat), dim=0)
    px = torch.cat((target_img_feat, px), dim=0)
    px = torch.cat((poison_imgs_feat, px), dim=0)
    px = torch.cat((poison_init_feat, px), dim=0)
    px = torch.cat((antidote_imgs_feat, px), dim=0)
    px = torch.cat((antidote_init_feat, px), dim=0)
    print(px.shape)
