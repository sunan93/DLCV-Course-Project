...
class ResNet(nn.Module):
    ...
    def get_block_feats(self, x, multi):
        feat_list = []
        ...
        out_2 = self.layer2(out)
        # feat_list.append(out)
        if multi==True:
            for nl, layer in enumerate(self.layer2):
                out = layer(out)
                if len(self.layer2) - nl - 1 <= self.middle_feat_num and len(self.layer2) - nl - 1 > 0:
                    feat_list.append(out)


        out = self.layer3(out_2)
        # feat_list.append(out)
    ...
    def forward(self, x, penu=False, block=False, multi=False):
        if block:
            return self.get_block_feats(x, multi)
