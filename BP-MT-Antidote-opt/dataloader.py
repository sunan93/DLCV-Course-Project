...
class PoisonedDataset(data.Dataset):
    def __init__(self, path, subset='clean_train', transform=None, num_per_label=-1, poison_tuple_list=[], poison_indices=[],
                 antidote_tuple_list=[], antidote_indices=[],
                 class_labels=[i for i in range(10)], subset_group=0, load_poisons=0):
        """
        ...
        self.antidote_indices = antidote_indices
        self.antidote_tuple_list = antidote_tuple_list
        self.load_poisons = load_poisons
        self.get_valid_indices(num_per_label, poison_indices, antidote_indices, class_labels, subset_group, load_poisons)

    def get_valid_indices(self, num_per_label, poison_indices, antidote_indices, class_labels, subset_group, load_poisons):
        ...
        if load_poisons==1:
            ...
            # put antidotes into the dataset
            for pidx in antidote_indices:
                # count the poisoned class
                print("antidote index = ", pidx)
                img, label = self.img_label_list[pidx]
                if label not in class_labels:
                    continue
                # reduce the number of poison classes
        ...
        print(len(self.valid_indices), self.valid_indices, antidote_indices, poison_indices)


    def __len__(self):
        if self.load_poisons==1:
            return len(self.valid_indices) + len(self.poison_tuple_list) + len(self.antidote_tuple_list)
        else:
    ...
    def __getitem__(self, index):
        if self.load_poisons==1:
            ...
            elif index < len(self.poison_tuple_list) + len(self.antidote_tuple_list):
                img, label = self.antidote_tuple_list[index - len(self.poison_tuple_list)]

            else:
                idx = self.valid_indices[index - len(self.poison_tuple_list) - len(self.antidote_tuple_list)]
