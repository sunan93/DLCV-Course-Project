...
class _Witch():
    ...
    def _initialize_brew(self, victim, kettle):
        ...
        self.intended_classes = torch.as_tensor( [kettle.poison_setup['intended_class'][0] for x in range(len(self.targets))], device=self.setup['device'], dtype=torch.long)
        print("intended classes", self.intended_classes)
        print("true classes", self.true_classes)
