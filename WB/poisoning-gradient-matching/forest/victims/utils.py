...
def print_and_save_stats(epoch, stats, current_lr, train_loss, train_acc, valid_acc, valid_loss,
    ...
    if valid_acc is not None:
        ...
        stats['target_accs_unseen'].append(target_acc_unseen)
        stats['target_losses_unseen'].append(target_loss_unseen)
        stats['target_accs_clean_unseen'].append(target_clean_acc_unseen)
        stats['target_losses_clean_unseen'].append(target_clean_loss_unseen)
        print(f'Epoch: {epoch:<3}| lr: {current_lr:.4f} | '
              f'Unseen Target adv. loss is {target_loss:7.4f}, unseen fool  acc: {target_acc:7.2%} | '
              f'Unseen Target orig. loss is {target_clean_loss:7.4f}, unseen orig. acc: {target_clean_acc:7.2%} | ')
