...
def run_step(kettle, poison_delta, loss_fn, epoch, stats, model, defs, criterion, optimizer, scheduler, ablation=True):
    ...
    if epoch % defs.validate == 0 or epoch == (defs.epochs - 1):
        ...
        target_acc, target_loss, target_clean_acc, target_clean_loss, target_acc_unseen, target_loss_unseen, target_clean_acc_unseen, target_clean_loss_unseen = check_targets(
            model, criterion, kettle.targetset, kettle.poison_setup['intended_class'],
            kettle.poison_setup['target_class'],
            kettle.setup, kettle.unseen_targetset)
    else:
        valid_acc, valid_loss = None, None
        target_acc, target_loss, target_clean_acc, target_clean_loss = [None] * 4
        target_acc_unseen, target_loss_unseen, target_clean_acc_unseen, target_clean_loss_unseen = [None] * 4
    ...
    print_and_save_stats(epoch, stats, current_lr, epoch_loss / (batch + 1), correct_preds / total_preds,
                         ...
                         target_acc_unseen, target_loss_unseen, target_clean_acc_unseen, target_clean_loss_unseen)
...
def check_targets(model, criterion, targetset, intended_class, original_class, setup, unseen_targetset):
    """Get accuracy and loss for all targets on their intended class."""
    model.eval()
    if len(targetset) > 0:
        #print(intended_class)
        target_images = torch.stack([data[0] for data in targetset]).to(**setup)
        unseen_target_images = torch.stack([data[0] for data in unseen_targetset]).to(**setup)
        #intended_labels = torch.tensor(intended_class).to(device=setup['device'], dtype=torch.long)
        intended_labels = torch.as_tensor( [intended_class[0] for x in range(len(target_images))], device=setup['device'], dtype=torch.long)
        intended_labels_unseen = torch.as_tensor( [intended_class[0] for x in range(len(unseen_target_images))], device=setup['device'], dtype=torch.long)
        original_labels = torch.stack([torch.as_tensor(data[1], device=setup['device'], dtype=torch.long) for data in targetset])
        original_labels_unseen = torch.stack([torch.as_tensor(data[1], device=setup['device'], dtype=torch.long) for data in unseen_targetset])
        #intended_labels = torch.zeros([len(target_images)]).to(setup['device']).long()
        #intended_labels[:] = intended_class
        with torch.no_grad():
            outputs = model(target_images)
            predictions = torch.argmax(outputs, dim=1)
            print("outputs ", outputs)
            print("intended labels ", intended_labels)
            ...
            outputs_unseen = model(unseen_target_images)
            predictions_unseen = torch.argmax(outputs_unseen, dim=1)
            #print("outputs unseen", outputs_unseen)
            #print("intended labels unseen ", intended_labels_unseen)
            loss_intended_unseen = criterion(outputs_unseen, intended_labels_unseen)
            accuracy_intended_unseen = (predictions_unseen == intended_labels_unseen).sum().float() / predictions_unseen.size(0)
            loss_clean_unseen = criterion(outputs_unseen, original_labels_unseen)
            predictions_clean_unseen = torch.argmax(outputs_unseen, dim=1)
            accuracy_clean_unseen = (predictions_unseen == original_labels_unseen).sum().float() / predictions_unseen.size(0)


        return accuracy_intended.item(), loss_intended.item(), accuracy_clean.item(), loss_clean.item(), accuracy_intended_unseen.item(), loss_intended_unseen.item(), accuracy_clean_unseen.item(), loss_clean_unseen.item()
