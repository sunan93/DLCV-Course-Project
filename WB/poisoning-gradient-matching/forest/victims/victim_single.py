...
class _VictimSingle(_VictimBase):
    ...
    def re_initialize(self, seed=None, model_to_load=""):
        print("inside re_initialize method")
        """
        ...
        if model_to_load=="MobileNetV2":
            self.model.load_state_dict(torch.load('./MobileNetV2_cifar.pth')) # remove when clean training model
        else:
            self.model.load_state_dict(torch.load('./Resnet50_cifar.pth')) # remove when clean training model
    ...
    def initialize(self, seed=None, model_to_load=""):
        print("inside initialize method")
        if self.args.modelkey is None:
            if seed is None:
                self.model_init_seed = np.random.randint(0, 2**32 - 1)
            else:
        ...
        if model_to_load=="MobileNetV2":
            ...
            #self.model.load_state_dict(torch.load('./Resnet50_cifar.pth')) # remove when clean training model
            self.model.load_state_dict(torch.load('./cifar100_model.pth')) # remove when clean training model
        
        #if torch.cuda.device_count() > 1:
        #    self.model = torch.nn.DataParallel(self.model)
        
        #self.model.load_state_dict(torch.load('./cifar_model.pth')) # remove when clean training model
        print(f'{self.args.net[0]} model initialized with random key {self.model_init_seed}.')
