...
def construct_datasets(dataset, data_path, normalize=True):
    ...
    if normalize:
        ...
        carset.data_mean = data_mean
        carset.data_std = data_std
    else:
    ...
    carset.transform = transform_train
...
class CarDataset():
    def __init__(self):
        ...
        #])
        root_path = "/sdb/sunandini/val_project/BullseyePoison/datasets/epfl-gims08/resized/tripod_seq"
        labels_list = []
        while True:
            path = '{}_{}_{}.jpg'.format(root_path, '%.2d' % subset, '%.3d' % idx)
            print("append image ", path)
            if os.path.exists(path):
                img = Image.open(path)
                #target_list.append(np.array(transforms_test(img)[None, :, :, :]))
                print(img)
                target_list.append(np.asarray(img))
                #labels_list.append(np.array([target_label]))
        ...
        print(len(target_list), target_list[0].shape)
        self.data = np.vstack(target_list).reshape(-1, 32, 32, 3)
        #self.data = self.data.transpose((0, 2, 3, 1)) 
        print("data.shape ", self.data.shape)
        #self.targets = np.vstack(labels_list)
        #print("targets.shape ", self.targets.shape)

        self.targets = labels_list
        print("targets.shape ", self.targets)
...
class CarDataset():
    def __init__(self):
        ...
        #])
        import os
        root_path = "/sdb/sunandini/val_project/obama_pictures/"
        arr = os.listdir(root_path)
        labels_list = []
        for i in range(len(arr)):
            print("append image ", arr[i])
            img = Image.open(root_path + arr[i])
            #target_list.append(np.array(transforms_test(img)[None, :, :, :]))
            print(img)
            target_list.append(np.asarray(img))
            #labels_list.append(np.array([target_label]))
    ...
    def get_target(self, index):
        ...
        """
        img, target = self.data[index], self.targets[index]
        #print("Cifar img", self.data.shape, type(self.data), img.shape, img)
...
class CIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        """
        img, target = self.data[index], self.targets[index]
        #print("Cifar img", type(img), img.shape)
        # doing this so that it is consistent with all other datasets
        # to return a PIL Image
        img = Image.fromarray(img)
    ...
    def get_target(self, index):
        ...
        """
        #img = self.data[index]
        #print("Cifar img", self.data.shape, type(self.data), len(self.targets), type(self.targets))
