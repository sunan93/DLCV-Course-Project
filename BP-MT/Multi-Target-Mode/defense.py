...
def load_pretrained_net_2(net_name, chk_name, model_chk_path, test_dp=0, bdp=0, device='cuda'):
    """
    ...
    if device == 'cuda':
        ...
        checkpoint = torch.load("/sdb/sunandini/val_project/BP_dup/BullseyePoison/Multi-Target-Mode/last_epoch.pth")
    else:
        ...
        for ite in ites[::-1]:
            ...
            for victim_name in args.target_net:
                ...
                if True:
                    ...
                    victim_net = load_pretrained_net_2(victim_name, args.test_chk_name, model_chk_path=args.model_resume_path,
                                                     device=args.device)
                ...
                for k in range(1, 21):
                    ...
                    print("reduction in length ", len(poison_filtered_tuple_list), len(poison_tuple_list))
                    reductions += (len(poison_tuple_list) - len(poison_filtered_tuple_list))
                ...
                acc = avg_attack_acc/20.0
                reductions = reductions*100 / (20.0*5)
                print("reduction in attack acc", avg_attack_acc/20.0, attack_acc)
                print("reduction in poisons list", reductions)
                """
