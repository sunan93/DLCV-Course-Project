...
if __name__ == '__main__':
    ...
    if args.eval_poison_path != "":
        ...
        poison_tuple_list = \
            make_convex_polytope_poisons(sub_net_list, victim_net, base_tensor_list, targets,
                                         ...
                                         feat_reg_flag=args.feat_reg_flag,ce_loss_flag=args.ce_loss_flag, multi_flag=args.multi_flag )
    ...
    for vnet, vnet_name in zip(targets_net, args.victim_net):
        ...
        for i in range(num_trials):
            print(vnet_name)
            attack_acc, attack_val_acc, pre_test_acc, post_test_acc = train_network_with_poison(vnet, targets, targets_indices, poison_tuple_list,
    ...
    for vnet, vnet_name in zip(targets_net, args.victim_net):
        ...
        poisoned_dset_filtered, poison_filtered_tuple_list, deleted_poisons_base_indices, num_deleted_samples = \
                        knn_defense(vnet, poison_tuple_list, base_idx_list, poisoned_dset, 5)
        ...
        attack_acc, attack_val_acc, pre_test_acc, post_test_acc = train_network_with_poison(vnet, targets, targets_indices, poison_filtered_tuple_list,
                                                                   ...
                                                                   args, save_state=False, eval_targets=eval_targets)
        res.append((attack_acc, attack_val_acc, pre_test_acc, post_test_acc, (args.poison_num - len(filtered_list))*100/args.poison_num)) 


    print("------SUMMARY POST DEFENSE------")
    print("TIME ELAPSED (mins): {}".format(int((tt - t) / 60)))
    print("TARGET INDEX: {}".format(args.target_index))
    for tnet_name, (attack_acc, attack_val_acc, pre_test_acc, post_test_acc, poison_red_percent) in zip(args.victim_net, res):
        print(tnet_name, attack_acc, attack_val_acc, pre_test_acc, post_test_acc, poison_red_percent)
