...
def load_pretrained_net(net_name, chk_name, model_chk_path, test_dp=0, bdp=0, device='cuda'):
    ...
    if device == 'cuda':
        ...
        print('./{}/{}'.format(model_chk_path, chk_name) % net_name)
    else:
...
def get_targets_feat_list(subs_net_list, targets, device, multi_flag=False, end2end=False):
    targets_feat_list = []
    num = 20
    ...
    for n, net in enumerate(subs_net_list):
        ...
        for target in targets:
            ...
            if end2end:
                feats = [feat.detach() for feat in net(x=target, block=True, multi=multi_flag)]
            else:
...
def get_target_nearest_neighbor(subs_net_list, target_cls_imgs, target_img, num_imgs, idx_list, device='cuda'):
    ...
    target_img.resize_(len(target_img), 3, 32, 32)
    target_cls_imgs = target_cls_imgs.to(device)
    target_cls_imgs.resize_(len(target_cls_imgs), 1, 3, 32, 32)
    print(target_cls_imgs.shape, target_img.shape)
    total_dists = np.zeros(len(target_cls_imgs))
    with torch.no_grad():
        for n_net, net in enumerate(subs_net_list):
            target_img_feat = net.module.penultimate(target_img)
            dist_arr = []
            for i in range(len(target_cls_imgs)):
                target_cls_imgs_feat = net.module.penultimate(target_cls_imgs[i])
                dists = torch.sum((target_img_feat-target_cls_imgs_feat)**2).cpu().detach().numpy()
                dist_arr.append(dists)
            total_dists += dist_arr
    min_dist_idxes = np.argsort(total_dists)
    # print("Selected feature dist squares: {}".format(total_dists[min_dist_idxes[:num_imgs]]))
    target_cls_imgs.resize_(len(target_cls_imgs), 3, 32, 32)
