...
class PoisonBatch2(torch.nn.Module):
    """
    Implementing this to work with PyTorch optimizers.
    """

    def __init__(self, base_batch):
        super(PoisonBatch2, self).__init__()
        self.poison = torch.nn.Parameter(base_batch.clone())
...
def loss_from_center(subs_net_list, target_feat_list, poison_batch, net_repeat, end2end, max_loss_flag=False, feat_reg_flag=False, ce_loss_flag=False, multi_flag=False, device='cuda'):
    if end2end:
        loss = 0
        loss_arr = []
        loss_coef = []
...
def lpips_dist(x, x_new, device, net, multi_flag, std_cifar, mean_cifar, loss_fn_alex):
    """
    poison_new = PoisonBatch2(x_new).to(device)
    poison_old = PoisonBatch2(x).to(device)
    new_poison_feats = net(x=poison_new(), block=True, multi=multi_flag)
    orig_poison_feats = net(x=poison_old(), block=True, multi=multi_flag)
    dist=0
    for pfeat_old, pfeat_new in zip(orig_poison_feats, new_poison_feats):
        pfeat_old = pfeat_old.to(device)
        pfeat_new = pfeat_new.to(device)
        diff = torch.mean(pfeat_old - pfeat_new, dim=0)
        diff_norm = torch.norm(diff, dim=0)
        dist += torch.mean(diff_norm)
    """
    std = torch.tensor((0.5, 0.5, 0.5)) 
    mean = torch.tensor((0.5, 0.5, 0.5))
    #print(x.shape, x_new.shape)
    dist = 0
    x = x * std_cifar + mean_cifar 
    x_new = x_new * std_cifar + mean_cifar
    # range [0,1] to [-1, 1]
    x = (x-0.5)/0.5
    x_new = (x_new-0.5)/0.5
    #import lpips
    ...
    for i in range(len(x)):
        dist += loss_fn_alex(x[i].cpu(), x_new[i].cpu())
    dist = dist/len(x)
    #print(dist)
    return dist
...
def make_convex_polytope_poisons(subs_net_list, victim_net, base_tensor_list, targets, targets_indices, device,
                                 ...
                                 net_repeat=1, max_loss_flag=False, feat_reg_flag=False, ce_loss_flag=False, multi_flag=False):
    ...
    for ite in range(start_ite, iterations):
        ...
        if mode == 'convex':
            ...
            if feat_reg_flag==True:
                ...
                total_loss = loss_from_center(subs_net_list, targets_center_list, poison_batch, net_repeat, end2end, False, False, False, multi_flag)
            coeffs_time_tmp = 0
        ...
        alpha_max=1
        num_steps = 20
        x_cap = poison_batch.poison.data
        x = base_tensor_batch
        dist_threshold = 0.05
        #print(torch.min(x_cap), torch.max(x_cap), torch.min(x), torch.max(x))
        delta = x_cap - x
        #print(torch.mean(delta)) 
        for steps in range(num_steps):
            alpha = (alpha_min + alpha_max)/2
            x_new = x + delta*alpha
            dist = lpips_dist(x, x_new, device, victim_net, multi_flag, std, mean, loss_fn_alex)
            #print(dist, alpha)
            if dist>dist_threshold:
                alpha_max = alpha
            else:
                alpha_min = alpha
        poison_batch.poison.data = x_new 

        #perturb_range01 = torch.clamp((poison_batch.poison.data - base_tensor_batch) * std, -epsilon, epsilon)
        #print((poison_batch.poison.data - base_tensor_batch))
        ...
        if ite % 50 == 0 or ite == iterations - 1:
            ...
            print(torch.mean(poison_batch.poison.data - base_tensor_batch))
            sys.stdout.flush()
